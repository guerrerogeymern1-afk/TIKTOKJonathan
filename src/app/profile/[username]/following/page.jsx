...
import { ChevronLeft, Search, UserCheck, UserPlus, Users } from 'lucide-react';
...
export default function FollowingList() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data } = await supabase
        .from('followers')
        .select('following_id, profiles!followers_following_id_fkey(id, username, avatar_url, full_name)')
  ...
  useEffect(() => {
    if (!query.trim()) { setFiltered(following); return; }
    setFiltered(following.filter(u => u.username?.toLowerCase().includes(query.toLowerCase())));
  }, [query, following]);
  ...
  return (
    <div className={`flex flex-col h-full w-full transition-colors duration-300 ${isDark ? 'bg-[#0a0a0a] text-white' : 'bg-white text-black'}`}>
      <div className={`flex items-center gap-3 px-4 py-4 border-b sticky top-0 z-10 ${isDark ? 'border-white/5 bg-[#0a0a0a]' : 'border-black/5 bg-white'}`}>
        ...
        <h1 className="text-lg font-black">@{username} sigue a</h1>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 gap-3 opacity-30 text-center">
            ...
            <p className="font-medium">No sigue a nadie{query ? ' que coincida' : ' todavía'}</p>
          </div>
        ) : (
